...
void _output_file_3(Node** pq, char* output)
{
	...
	while((*pq) -> next != NULL)
	{
		Node* pop_node_1 = _pq_dequeue(pq, &pop_node_1);
		Node* pop_node_2 = _pq_dequeue(pq, &pop_node_2);
	...
	char* path = malloc(sizeof(*path) * (*pq) -> tree -> amount);
	 ...
	 _print_output3(fp_output, (*pq) -> tree, path, 0);

	fclose(fp_output);

	free(path);
}
void _print_output3(FILE* fp, TreeNode* tree, char* path, int i)
{
	if(tree -> ascii != 260)
	{
	...
	{
		path[i] = '0';
		_print_output3(fp, tree -> left, path, i + 1);
		path[i] = '1';
		_print_output3(fp, tree -> right, path, i + 1);
	}	
}
